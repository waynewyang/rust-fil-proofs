...
// value is just an approximation, it's not actually measured. Special
// values that override the size meaning are: `0`, disable cache, and
// any negative number, unlimited size.
pub const MAX_CACHE_SIZE: isize = -1;
// FIXME: Profile cache use to check if this value is being correctly
// approximated and recalibrate.
    ...
    G: Graph<H>,
{
    pub fn new(
        ...
        let cache_entries = if MAX_CACHE_SIZE < 0 {
            info!(SP_LOG, "using parents cache of unlimited size",);
            // FIXME: Even here, estimate cache size to report it to the user.
            nodes
        } else if MAX_CACHE_SIZE == 0 {
            0
        // FIXME: Not really handled as a special case, when we're not using
        // this cache we shouldn't even create `parents_cache`.
        } else {
            // `MAX_CACHE_SIZE` > 0: estimate the maximum number of entries
            // the parents cache can have.
            let size_bytes = (MAX_CACHE_SIZE as usize) << 20;
